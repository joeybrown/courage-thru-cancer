...
export class Card {
  ...
  private videoFrame: (height: number, width: number) => string = (height, width) => {
    return `<iframe width="${width}" height="${height}" src="https://www.youtube.com/embed/${this.youtubeSlug}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>`;
  }
...
export class CardList {
  ...
  static renderFilteredVideos(videos: Video[]) : string {
    let html = '<div>';
    const years = [...new Set(videos.map((item) => item.year))]
    for (const year of years) {
      html += `<h2>${year}</h2>`;
